  ...
  if (JSONpayload.method === "playGame") {
    ...
    const playGame = (game) => {
      ...
      while (playerWantCards > 0) {
        for (const player of database.players.slice(
          0,
          database.listPlayers.length - 1
        )) {
          let valid = 1;
          while (valid === 1) {
            if (player.score > 21) {
              ...
              const answer = prompt(
                `${player.name}, do you want one more card [y/n]: `
              );
              if (answer === "y") {
                database.nextCard(player);
                valid = 0;
                console.log(
                  `${player.name} received a ${
      ...
      for (const player of database.players.slice(
        ...
        if (player.score > 21) {
          ...
          console.log(
            `${player.name} wins with score ${player.score}.\n ${
              database.players[database.players.length - 1].name
            } is BUST with score ${
              database.players[database.players.length - 1].score
            }.`
          );
        } else if (
          player.score > database.players[database.players.length - 1].score
        ) {
          console.log(
            `${player.name} wins with score ${player.score}.\n ${
              database.players[database.players.length - 1].name
            } has score ${database.players[database.players.length - 1].score}.`
          );
        } else {
          console.log(
            `${player.name} has lost with score ${player.score}.\n ${
              database.players[database.players.length - 1].name
            } has score ${database.players[database.players.length - 1].score}.`
          );
        }
